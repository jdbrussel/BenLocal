<template>
    <AppLayout>
        <div class="max-w-7xl mx-auto p-6">
            <h1 class="text-3xl font-bold mb-8 tracking-tight">{{ $t('ui.search.title') }}</h1>

            <div class="relative mb-8">
                <SearchIcon class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                <input v-model="searchQuery" type="text" :placeholder="$t('ui.search.placeholder')"
            ...
            <div v-if="searchQuery.length >= 2" class="space-y-8">
                ...
                <div v-else>
                    ...
                    <section v-if="results.spots.length > 0" class="mb-8">
                        <h2 class="text-sm font-bold uppercase tracking-wider text-gray-500 mb-4">{{ $t('ui.common.spots') }}</h2>
                        <div class="space-y-4">
                    ...
                    <section v-if="results.regions.length > 0" class="mb-8">
                        <h2 class="text-sm font-bold uppercase tracking-wider text-gray-500 mb-4">{{ $t('ui.common.regions') }}</h2>
                    ...
                    <section v-if="results.users.length > 0">
                        <h2 class="text-sm font-bold uppercase tracking-wider text-gray-500 mb-4">{{ $t('ui.common.people') }}</h2>
                        <div class="space-y-3">
                    ...
                    <div v-if="!results.spots.length && !results.regions.length && !results.users.length" class="text-center py-8">
                        <p class="text-gray-500">{{ $t('ui.search.no_results') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div v-else>
                <section class="mb-8">
                    <h2 class="text-sm font-bold uppercase tracking-wider text-gray-500 mb-4">{{ $t('ui.search.recent_searches') }}</h2>
                    <div class="flex flex-wrap gap-2">
                ...
                <section>
                    <h2 class="text-sm font-bold uppercase tracking-wider text-gray-500 mb-4">{{ $t('ui.search.trending_searches') }}</h2>
                    <div class="space-y-4">
                        <div v-for="i in 3" :key="i" class="flex items-center gap-4 p-2">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500">{{ $t('ui.discover.trending_searching', { count: '2.4k' }) }}</p>
                            </div>
                        </div>
                    </div>
                </section>
